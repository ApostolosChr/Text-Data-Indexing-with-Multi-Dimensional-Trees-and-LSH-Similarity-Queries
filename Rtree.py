...
import numpy as np
import nltk
from pykdtree.kdtree import KDTree as RTree
...
def filter_scientists(combined_data, rtree, min_awards, min_dblp, initial_letters):
    ...
    for letter in range(ord(initial_range[0]), ord(initial_range[-1]) + 1):
        ...
        closest_indices = rtree.query(sample, k=len(combined_data))[1][0]
...
while True:
    ...
    filtered_scientists = filter_scientists(combined_data, rtree, min_awards, min_dblp, initial_letters)
